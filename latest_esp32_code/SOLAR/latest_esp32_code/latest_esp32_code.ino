...
void setup() {
  ...
  // Check and create file for readings
  if (!SD.exists("/pzemReadings.csv")) {
    writeFile(SD, "/pzemReadings.csv", pzemHeaders.c_str());
  }
...
void loop() {
  ...
  if (arduinoESPSerial.available()) {
    ...
    appendFile(SD, "/pzemReadings.csv", (data + "\n").c_str());
